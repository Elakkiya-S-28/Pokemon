...
// Fixed: Random values are generated in useEffect to prevent hydration errors.
export const GrassBlade = ({ index }: { index: number }) => {
    const [style, setStyle] = useState<{ height: number; duration: number; delay: number } | null>(null);

    useEffect(() => {
        // Random values generated ONLY on the client
        setStyle({
    ...
    // Placeholder for Server Side Rendering (SSR) to match HTML structure
    if (!style) {
        return <div className="absolute bottom-0 w-[2px] opacity-0" style={{ left: `${(index / 60) * 100}%` }} />;
    }
...
const CHAR_THEMES: Record<string, any> = {
    ash: {
...
export const WalkingSprite = ({
    ...
    return (
        <motion.div
            className="absolute bottom-6 md:bottom-12 left-0"
            initial={{ x: "-20vw" }}
            animate={{ x: "110vw" }}
            transition={{
                duration: speed,
        ...
        >
            ...
            >
                ...
                <g transform="translate(16, 20)">
                    ...
                    {isPika && <ellipse cx="16" cy="25" rx="14" ry="12" fill={theme.skin} />}
                </g>
                ...
                <g transform="translate(32, 22)">
                    ...
                    {type === 'misty' && (
                        <motion.path d="M10 -5 L20 -12 L18 -2 Z" fill={theme.hair} animate={{ rotate: [0, 15, 0] }} transition={{ duration: 0.6, repeat: Infinity }} />
                    )}
                    {type === 'ash' && (
                        <g>
                ...
                {isPika && (
                    <motion.path d="M15 45 L5 35 L15 35 L10 25" fill="none" stroke="#b45309" strokeWidth="3" animate={{ x: [0, 2, 0] }} />
                )}
            </motion.svg>
        </motion.div>
    );
